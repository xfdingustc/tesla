...
public class VdtCamera implements VdtCameraCmdConsts {
    ...
    public VdtCamera(VdtCamera.ServiceInfo serviceInfo, final OnConnectionChangeListener listener) {
        ...
        mCommunicationBus = new VdtCameraCommunicationBus(mAddress, new VdtCameraCommunicationBus.ConnectionChangeListener() {
            ...
            public void onConnectionFailed() {
                if (listener != null) {
                    listener.onConnectionFailed(VdtCamera.this);
                }
            }
...
//            Logger.t(TAG).d(String.format("cmd_Rec_GetMarkTime: p1: %s, p2: %s", p1, p2));
    ...
    private void ack_syncTime(String p1, String p2) {
        Logger.t(TAG).d(String.format("%s, %s"), p1, p2);
        int ret = Integer.parseInt(p1);
        if (ret == 0) {
            mRxBus.post(new NetworkEvent(NetworkEvent.NETWORK_EVENT_WHAT_SYNCTIME));
        }
    }

    private void ack_getDeviceTime(String p1, String p2) {
        Logger.t(TAG).d(String.format("%s, %s"), p1, p2);
        int ret1 = Integer.parseInt(p1);
        int ret2 = Integer.parseInt(p2);
        mRxBus.post(new NetworkEvent(NetworkEvent.NETWORK_EVENT_WHAT_GETTIME, ret1, ret2));
    }
